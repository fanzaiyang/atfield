...
import cn.fanzy.breeze.sqltoy.core.utils.DataSourceUtils.Dialect;
...
public class SqlScriptLoader {
	 ...
	 * @throws Exception
	 */
	public void initialize(SqlToyContextProperties properties) throws Exception {
		// 增加路径验证提示,最易配错导致无法加载sql文件
		this.sqlResourcesDir=properties.getSqlResourcesDir();
		...
		initialized = true;
		boolean enabledDebug = log.isDebugEnabled();
		try {
			// 检索所有匹配的sql.xml文件
			...
			if (realSqlList != null && !realSqlList.isEmpty()) {
				...
				if (enabledDebug) {
					log.debug("总计将加载.sql.xml文件数量为:{}", realSqlList.size());
					log.debug("如果.sql.xml文件不在下列清单中,很可能是文件没有在编译路径下(bin、classes等),请仔细检查!");
				} else {
				...
				if (repeatSqlSize > 0) {
					...
					if (properties.isBreakWhenSqlRepeat()) {
					...
					if (properties.isBreakWhenSqlRepeat()) {
						log.error(repeatSqlIds.toString());
					} else {
						log.warn(repeatSqlIds.toString());
					}
					if (properties.isBreakWhenSqlRepeat()) {
				...
				if (enabledDebug) {
					log.debug("总计加载*.sql.xml文件数量为:0 !");
					log.debug("请检查配置项sqlResourcesDir={}是否正确(如:字母拼写),或文件没有在编译路径下(bin、classes等)!", sqlResourcesDir);
				} else {
			...
			log.error("加载和解析以sql.xml结尾的文件过程发生异常!" + e.getMessage(), e);
			throw e;
		}
		...
		if (realSqlList != null && !realSqlList.isEmpty()) {
			...
			if (properties.getScriptCheckIntervalSeconds() == null) {
				...
				sleepSeconds = properties.getScriptCheckIntervalSeconds().intValue();
			}
			// update 2019-08-25 增加独立的文件变更检测程序用于重新加载sql
			if (sleepSeconds > 0 && sleepSeconds <= maxWait) {
				if (enabledDebug) {
					log.debug("已经开启sql文件变更检测，会自动间隔:{}秒检测一次,发生变更会自动重新载入!", sleepSeconds);
				} else {
				...
				watcher = new SqlFileModifyWatcher(sqlCache, filesLastModifyMap, realSqlList, dialect, encoding,
						properties.getDelayCheckSeconds(), sleepSeconds);
				watcher.start();
			} else {
				log.warn("sql文件更新检测:sleepSeconds={} 小于1秒或大于24小时，表示关闭sql文件变更检测!", sleepSeconds);
			}
		}
	}
	...
	public void putSqlToyConfig(SqlToyConfig sqlToyConfig) throws Exception {
		if (sqlToyConfig == null || StringUtil.isBlank(sqlToyConfig.getId())) {
			log.warn("sqlToyConfig is null 或者 id 为null!");
			return;
		}
		// 判断是否已经存在，存在则清理一下分页优化的缓存
		if (sqlCache.containsKey(sqlToyConfig.getId())) {
			log.warn("发现重复的SQL语句:id={} 将被覆盖!", sqlToyConfig.getId());
			// 移除分页优化缓存
