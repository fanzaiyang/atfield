...
import cn.fanzy.breeze.sqltoy.core.translate.model.CacheCheckResult;
...
import cn.fanzy.breeze.sqltoy.core.utils.HttpClientUtils;
...
public class TranslateFactory {
	...
	private static List doRestCheck(final SqlToyContext sqlToyContext, final CheckerConfigModel checkerConfig,
		...
		boolean fatal = false;
		try {
			result = JSONUtil.toList(jsonStr, CacheCheckResult.class);
		} catch (Exception e) {
			fatal = true;
		}
		if (fatal) {
			try {
				result = JSONUtil.toList(jsonStr, Object[].class);
				fatal = false;
			} catch (Exception e) {
				fatal = true;
			}
	...
	private static List<Object[]> getRestCacheData(final SqlToyContext sqlToyContext, TranslateConfigModel cacheModel,
		...
		JSONArray jsonSet = JSONUtil.parseArray(jsonStr);
		if (jsonSet.isEmpty()) {
			return null;
		}
