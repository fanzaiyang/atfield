...
public class BreezeAdminAttachmentServiceImpl implements BreezeAdminAttachmentService {
    ...
    public JsonContent<List<SysFile>> upload(String prefix, HttpServletRequest request) throws IOException {
        ...
        for (MultipartFile file : fileList) {
            ...
            SysFile one = sqlToyHelperDao.findOne(Wrappers.lambdaWrapper(SysFile.class).eq(SysFile::getIdentifier, md5)
    ...
    @Override
    public JsonContent<BreezePutMultipartFileResponse> uploadMultipartInit(BreezePutMultipartFileArgs args) {
    ...
    public JsonContent<SysFile> uploadMultipartMerge(String identifier, String minioConfigName) {
        BreezeMinioResponse response = breezeMultipartFileService.mergeChunk(identifier, minioConfigName);
