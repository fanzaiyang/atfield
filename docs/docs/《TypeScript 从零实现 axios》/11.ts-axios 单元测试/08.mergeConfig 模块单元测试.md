...
运行测试后我们发现 `mergeConfig.ts` 文件的分支覆盖率并未达到 100%，提示是 23 行，打开文件后发现最后一个 `else` 逻辑并未走到，也就是
`val1` 为 `undefined` 的情况。但实际上即使 `val1` 为 `undefined`，我们也是返回 `undefined`，也就是返回 `val1`
，所以这块代码的逻辑可以优化。
